...
class YTitleBar constructor(context: Context, attrs: AttributeSet? = null) : ViewGroup(context, attrs) {
    ...
    private fun getDefaultLeftView(): View {
        return ImageView(context).apply {
            ...
            val drawable = R.mipmap.titlebar_left_iv.getDrawable(context)
    ...
    override fun onAttachedToWindow() {
        ...
        if (mBgColor < 1 && !isInEditMode) {
            BarUtils.getActivityByView(this)?.let { BarUtils.setStatusBarColor(it, mBgColor) }
            BarUtils.getActivityByView(this)?.let { BarUtils.setStatusBarLightMode(it, BarUtils.isLightColor(mBgColor)) }
